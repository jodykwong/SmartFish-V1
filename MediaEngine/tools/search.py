...
class DuckDuckGoSearch:
    """
    DuckDuckGo 免费搜索客户端
    完全免费，无需 API Key，作为付费搜索引擎的替代方案
    """

    def __init__(self):
        """初始化 DuckDuckGo 客户端，无需 API Key"""
        try:
            ...
            try:
                ...
                raise ImportError("ddgs 库未安装，请运行 `pip install ddgs` 进行安装。")
        self._ddgs = DDGS()

    def _parse_search_response(self, results: List[Dict], query: str) -> BochaResponse:
        """将 DuckDuckGo 结果转换为 BochaResponse 格式"""
    ...
    def _search_internal(self, query: str, max_results: int = 10, timelimit: Optional[str] = None) -> BochaResponse:
        """内部搜索方法"""
        try:
            import time
            time.sleep(0.5)  # 添加延迟避免反爬
            results = list(self._ddgs.text(query, max_results=max_results, timelimit=timelimit))
            return self._parse_search_response(results, query)
        except Exception as e:
            logger.exception(f"DuckDuckGo 搜索失败: {str(e)}")
            return BochaResponse(query=query)

    def comprehensive_search(self, query: str, max_results: int = 10) -> BochaResponse:
        """【工具】综合搜索"""
        logger.info(f"--- TOOL: DuckDuckGo 综合搜索 (query: {query}) ---")
        return self._search_internal(query, max_results)

    def web_search_only(self, query: str, max_results: int = 15) -> BochaResponse:
        """【工具】纯网页搜索"""
        logger.info(f"--- TOOL: DuckDuckGo 纯网页搜索 (query: {query}) ---")
        return self._search_internal(query, max_results)

    def search_for_structured_data(self, query: str) -> BochaResponse:
        """【工具】结构化数据查询（DuckDuckGo 不支持模态卡，返回普通搜索结果）"""
        logger.info(f"--- TOOL: DuckDuckGo 结构化数据查询 (query: {query}) ---")
        return self._search_internal(query, 5)

    def search_last_24_hours(self, query: str) -> BochaResponse:
        """【工具】搜索24小时内信息"""
        logger.info(f"--- TOOL: DuckDuckGo 搜索24小时内信息 (query: {query}) ---")
        return self._search_internal(query, timelimit='d')

    def search_last_week(self, query: str) -> BochaResponse:
        """【工具】搜索本周信息"""
        logger.info(f"--- TOOL: DuckDuckGo 搜索本周信息 (query: {query}) ---")
        return self._search_internal(query, timelimit='w')
...
def load_agent_from_config():
    """根据配置文件选择并加载搜索Agent，优先级：Anspire > Bocha > DuckDuckGo（免费）"""
    if settings.ANSPIRE_API_KEY:
        ...
        logger.info("未配置付费搜索API，使用免费的 DuckDuckGoSearch Agent")
