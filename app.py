import os
# 【关键修复】禁用 eventlet 的 GreenDNS 系统。
# 它是导致与 httpx (OpenAI 客户端底层) 冲突并引发 DNS 解析超时 (Connection error) 的根本原因。
...
# eventlet 在客户端主动断开时偶尔会抛出 ConnectionAbortedError，已通过全局 monkey_patch 处理。
...
# ==================== SmartFish API ====================
# Story 2-1, 2-2, 3-1, 3-2: 模板和方法 API

@app.route('/api/templates', methods=['GET'])
def get_templates():
    """获取所有模板元数据列表"""
    try:
        from config import get_config as get_smartfish_config
        cfg = get_smartfish_config()
...
def get_methods():
    ...
    try:
        ...
        methods = cfg.get_methods(category)
        # 返回简化版本
        result = [{
            'id': m.get('id'),
            'name': m.get('name'),
        ...
        return jsonify({'success': True, 'methods': result})
    except Exception as e:
        logger.exception("获取方法列表失败")
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/methods/<method_id>', methods=['GET'])
def get_method_detail(method_id):
    ...
    try:
        ...
        return jsonify({'success': False, 'message': '方法不存在'}), 404
    except Exception as e:
        logger.exception("获取方法详情失败")
        return jsonify({'success': False, 'message': str(e)}), 500

@app.route('/api/method-categories', methods=['GET'])
def get_method_categories():
    """获取所有方法分类"""
    try:
        from config import get_config as get_smartfish_config
        cfg = get_smartfish_config()
        categories = cfg.get_method_categories()
        return jsonify({'success': True, 'categories': categories})
    except Exception as e:
        logger.exception("获取方法分类失败")
        return jsonify({'success': False, 'message': str(e)}), 500
