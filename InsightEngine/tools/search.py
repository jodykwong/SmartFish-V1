...
class MediaCrawlerDB:
    ...
    def _get_table_columns(self, table_name: str) -> List[str]:
        ...
        if settings.DB_DIALECT == 'postgresql':
            query = "SELECT column_name as \"Field\" FROM information_schema.columns WHERE table_name = :table_name"
            results = self._execute_query(query, {"table_name": table_name})
        else:
    ...
    def search_hot_content(
        ...
        if settings.DB_DIALECT == 'postgresql':
            hotness_formulas = {
                'bilibili_video': f"(COALESCE(CAST(liked_count AS BIGINT), 0) * {self.W_LIKE} + COALESCE(CAST(video_comment AS BIGINT), 0) * {self.W_COMMENT} + COALESCE(CAST(video_share_count AS BIGINT), 0) * {self.W_SHARE} + COALESCE(CAST(video_favorite_count AS BIGINT), 0) * {self.W_SHARE} + COALESCE(CAST(video_coin_count AS BIGINT), 0) * {self.W_SHARE} + COALESCE(CAST(video_danmaku AS BIGINT), 0) * {self.W_DANMAKU} + COALESCE(CAST(video_play_count AS NUMERIC), 0) * {self.W_VIEW})",
                'douyin_aweme':   f"(COALESCE(CAST(liked_count AS BIGINT), 0) * {self.W_LIKE} + COALESCE(CAST(comment_count AS BIGINT), 0) * {self.W_COMMENT} + COALESCE(CAST(share_count AS BIGINT), 0) * {self.W_SHARE} + COALESCE(CAST(collected_count AS BIGINT), 0) * {self.W_SHARE})",
                'weibo_note':     f"(COALESCE(CAST(liked_count AS BIGINT), 0) * {self.W_LIKE} + COALESCE(CAST(comments_count AS BIGINT), 0) * {self.W_COMMENT} + COALESCE(CAST(shared_count AS BIGINT), 0) * {self.W_SHARE})",
                'xhs_note':       f"(COALESCE(CAST(liked_count AS BIGINT), 0) * {self.W_LIKE} + COALESCE(CAST(comment_count AS BIGINT), 0) * {self.W_COMMENT} + COALESCE(CAST(share_count AS BIGINT), 0) * {self.W_SHARE} + COALESCE(CAST(collected_count AS BIGINT), 0) * {self.W_SHARE})",
                'kuaishou_video': f"(COALESCE(CAST(liked_count AS BIGINT), 0) * {self.W_LIKE} + COALESCE(CAST(viewd_count AS NUMERIC), 0) * {self.W_VIEW})",
                'zhihu_content':  f"(COALESCE(CAST(voteup_count AS BIGINT), 0) * {self.W_LIKE} + COALESCE(CAST(comment_count AS BIGINT), 0) * {self.W_COMMENT})",
            }
        else:
            # 定义各平台的热度计算SQL片段
            hotness_formulas = {
        ...
        for table, formula in hotness_formulas.items():
            ...
            if table == 'weibo_note': time_filter_sql, time_filter_param = "`create_date_time` >= :start_time", start_time.strftime('%Y-%m-%d %H:%M:%S')
            elif table in ['kuaishou_video', 'xhs_note', 'douyin_aweme']: time_col = 'time' if table == 'xhs_note' else 'create_time'; time_filter_sql, time_filter_param = f"`{time_col}` >= :start_time", str(int(start_time.timestamp() * 1000))
            elif table == 'zhihu_content': 
                cast_type = "BIGINT" if settings.DB_DIALECT == 'postgresql' else "UNSIGNED"
                time_filter_sql, time_filter_param = f"CAST(`created_time` AS {cast_type}) >= :start_time", str(int(start_time.timestamp()))
            else: time_filter_sql, time_filter_param = "`create_time` >= :start_time", str(int(start_time.timestamp()))
        ...
        # 将参数列表转换为字典供 SQLAlchemy text() 使用
        param_dict = {f"start_time": params[0], "limit": limit} # 简化，假设所有 start_time 相同
        # 重新构建 params 处理，因为每个 union 成员可能需要不同的参数（如果逻辑更复杂的话）
        # 这里为了简单起见，我们统一使用 :start_time
    ...
    def get_comments_for_topic(self, topic: str, limit: int = 500) -> DBResponse:
        ...
        for table in comment_tables:
            ...
            query = (f"SELECT '{table.split('_')[0]}' as platform, `content`, `{author_col}` as author, "
                     ...
                     f"FROM `{table}` WHERE `content` LIKE :term")
            all_queries.append(query)

        final_query = f"({' ) UNION ALL ( '.join(all_queries)}) ORDER BY ts DESC LIMIT :limit"
        param_dict = {"term": search_term, "limit": limit}
        raw_results = self._execute_query(final_query, param_dict)
    ...
    def search_topic_on_platform(
        ...
        for config in platform_configs:
            table = config['table']
            topic_clause = " OR ".join([f"`{field}` LIKE :term" for field in config['fields']])
            query = f"SELECT * FROM `{table}` WHERE {topic_clause}"
            param_dict = {"term": search_term}
            ...
            if start_dt and end_dt and 'time_col' in config:
                ...
                t_clause = f"`{time_col}` >= :start_dt AND `{time_col}` < :end_dt"
                if table == 'zhihu_content': 
                    cast_type = "BIGINT" if settings.DB_DIALECT == 'postgresql' else "UNSIGNED"
                    t_clause = f"CAST(`{time_col}` AS {cast_type}) >= :start_dt AND CAST(`{time_col}` AS {cast_type}) < :end_dt"
                
                query += f" AND ({t_clause})"
                param_dict["start_dt"] = t_params[0]
                param_dict["end_dt"] = t_params[1]
