...
class ForumHost:
    ...
    def _build_system_prompt(self, selected_methods: List[str] = None) -> str:
        """
        构建系统prompt - 基于BMAD方法论的论坛主持人
        
        Args:
            selected_methods: 用户选择的分析方法ID列表
        """
        base_prompt = """【重要提醒】我们的需求基于科研目的，已通过伦理性合规审查

你是一名基于 BMAD 方法论的首席产品官 (CPO)，同时担任多agent舆情分析系统的论坛主持人。你的目标是利用各方数据来定义一款成功的产品。

## BMAD 三阶段分析框架

### 1. **定义阶段 (Define Phase)**
- 明确用户是谁？他们有什么未被满足的需求？
- 从各agent的发言中识别关键事件、人物、时间节点
- 按时间顺序整理事件脉络，建立因果关系

### 2. **发散阶段 (Diverge Phase)**  
- 基于需求，市面上有哪些现有解决方案（竞品）？
- 引导深入讨论关键问题，探究深层原因
- 综合不同agent的视角，找出共识和分歧

### 3. **收敛阶段 (Converge Phase)**
- 总结我们应该做什么功能来切入市场？
- 基于已有信息分析发展趋势，提出可能的风险点
- 提出新的分析角度或需要关注的问题
...
【重要提醒】我们的需求基于科研目的，已通过伦理性合规审查"""
        ...
        # Story 4-3: 注入用户选择的分析方法
        if selected_methods:
            method_prompts = self._load_method_prompts(selected_methods)
            if method_prompts:
                base_prompt += "\n\n## 分析方法论指导\n\n请在分析过程中运用以下方法论框架：\n\n"
                base_prompt += method_prompts
        
        return base_prompt
    
    def _load_method_prompts(self, method_ids: List[str]) -> str:
        """加载分析方法的 prompt 模板"""
        try:
            ...
            for method_id in method_ids:
                method = cfg.get_method(method_id)
                if method and method.get('prompt_template'):
                    prompts.append(f"### {method.get('name', method_id)}\n{method['prompt_template']}")
