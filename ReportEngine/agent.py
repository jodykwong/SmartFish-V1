    ...
    def generate_report(self, query: str, reports: List[Any], forum_logs: str = "",
        ...
        try:
            ...
            for section in sections:
                ...
                # 判断是否需要分段生成（长章节）
                use_segmented = self.chapter_generation_node._should_use_segmented_generation(
                    section, generation_context
                )
                if use_segmented:
                    logger.info(f"章节 {section.title} 目标字数较多，启用分段生成策略")
                    emit('chapter_status', {
                        'chapterId': section.chapter_id,
                        'title': section.title,
                        'status': 'segmented_generation'
                    })

                while attempt <= chapter_max_attempts:
                    try:
                        if use_segmented:
                            chapter_payload = self.chapter_generation_node.run_segmented(
                                section,
                                generation_context,
                                run_dir=run_dir,
                                stream_callback=chunk_callback,
                                max_tokens=4096,  # 分段生成每次调用限制较小
                            )
                        else:
