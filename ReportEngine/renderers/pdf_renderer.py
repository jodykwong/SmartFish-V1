...
/* ========== PDF 高对比度样式 (WCAG AAA 7:1) ========== */
/* 参考: https://www.30secondsofcode.org/css/s/print-stylesheet/ */
/* 纯黑文字 #000000 在白底上对比度为 21:1 */
...
/* 次要文本使用深灰 #333 (对比度 12.6:1) */
.text-muted, .text-secondary, .subtitle, .hint, .caption, small {{
...
/* KPI 卡片 - 纯黑文字 */
.kpi-card, .kpi-value, .kpi-label, .kpi-change, .kpi-grid {{
    color: #000000 !important;
}}
.kpi-value {{
    font-weight: bold !important;
...
/* ========== WeasyPrint Grid 兼容性修复 ========== */
/* WeasyPrint 处理复杂 grid 布局时可能出现 advancements=None 的 bug */
/* 将复杂的 grid 布局降级为 flexbox */
...
"""
        ...
        # ========== WeasyPrint Grid Bug 终极修复 ==========
        # WeasyPrint 处理 CSS Grid 时存在 advancements=None 的 bug
        # 直接在 HTML 中移除所有 display: grid 样式，强制使用 flexbox
        import re
        # 移除内联样式中的 display: grid
        html = re.sub(r'display\s*:\s*grid\s*;?', 'display: flex; flex-wrap: wrap;', html)
        # 移除 grid-template-* 属性
        html = re.sub(r'grid-template-[^;:]+\s*:\s*[^;]+;?', '', html)
        # 移除 grid-auto-* 属性
        html = re.sub(r'grid-auto-[^;:]+\s*:\s*[^;]+;?', '', html)
        # 移除 grid-column/grid-row 属性
        html = re.sub(r'grid-(?:column|row)\s*:\s*[^;]+;?', '', html)
