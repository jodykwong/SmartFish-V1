...
class DocumentLayoutNode(BaseNode):
    ...
    def run(
        ...
        """
        # 截断过长的报告内容 (每个报告保留前2000字符)
        truncated_reports = {}
        if isinstance(reports, dict):
            for engine, content in reports.items():
                if len(content) > 2000:
                    truncated_reports[engine] = content[:2000] + "...(内容已截断)"
                else:
                    truncated_reports[engine] = content
        else:
            # 兼容列表格式
            truncated_reports = []
            for r in reports:
                if len(str(r)) > 2000:
                    truncated_reports.append(str(r)[:2000] + "...(内容已截断)")
                else:
                    truncated_reports.append(str(r))

        # 截断论坛日志 (保留前1500字符)
        truncated_forum = forum_logs
        if len(forum_logs) > 1500:
            truncated_forum = forum_logs[:1500] + "...(讨论内容已截断)"

        # 将模板原文、切片结构与多源报告一并喂给LLM，便于其理解层级与素材
        payload = {
            "query": query,
            "template": {
                "raw": template_markdown[:5000] if len(template_markdown) > 5000 else template_markdown,
