...
class ChapterGenerationNode(BaseNode):
    ...
    def _build_payload(self, section: TemplateSection, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        payload = {
            ...
            "reports": {
                "query_engine": reports.get("query_engine", "")[:10000] + ("...(截断)" if len(reports.get("query_engine", "")) > 10000 else ""),
                "media_engine": reports.get("media_engine", "")[:10000] + ("...(截断)" if len(reports.get("media_engine", "")) > 10000 else ""),
                "insight_engine": reports.get("insight_engine", "")[:10000] + ("...(截断)" if len(reports.get("insight_engine", "")) > 10000 else ""),
            },
            "forumLogs": context.get("forum_logs", "")[:5000] + ("...(截断)" if len(context.get("forum_logs", "")) > 5000 else ""),
        ...
        **kwargs,
        ...
        with self.storage.capture_stream(chapter_dir) as stream_fp:
            stream = self.llm_client.stream_invoke(
                ...
                max_tokens=kwargs.get("max_tokens"),  # 修复：传递max_tokens参数
            )
            for delta in stream:
    ...
    def _parse_chapter(self, raw_text: str) -> Dict[str, Any]:
        ...
        # 检测输出是否被截断（常见特征：JSON未闭合、末尾不是}或]）
        if cleaned and not cleaned.endswith(('}', ']', '```')):
            logger.warning("检测到LLM输出可能被截断，末尾字符: ...{}", cleaned[-50:] if len(cleaned) > 50 else cleaned)
    ...
    def _merge_fragment_sequences(self, blocks: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for block in blocks:
            # 防御性检查：跳过非字典类型或转换为 paragraph
            if not isinstance(block, dict):
                if block:  # 非空字符串转为 paragraph
                    merged.append(self._as_paragraph_block(str(block)))
                continue
    ...
    def _merge_nested_fragments(self, block: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 防御性检查：如果 block 不是字典，转换为 paragraph
        if not isinstance(block, dict):
            return self._as_paragraph_block(str(block) if block else "")
    ...
    def _should_use_segmented_generation(self, section: TemplateSection, context: Dict[str, Any]) -> bool:
        """
        判断是否需要使用分段生成策略。

        当前强制所有章节使用分段生成，避免 LLM 输出被截断。
        原有逻辑（基于字数阈值）已备份注释。

        参数:
            section: 当前章节模板切片。
            context: 全局上下文，包含篇幅规划。
        ...
        # chapter_plan_map = context.get("chapter_directives", {})
        # chapter_plan = chapter_plan_map.get(section.chapter_id, {}) if chapter_plan_map else {}
        # target_words = chapter_plan.get("targetWords", 0)
        # return target_words > 2500  # 阈值：超过 2500 字时启用分段生成
    ...
    def _generate_chapter_outline(
        ...
        参数:
            section: 当前章节模板切片。
            context: 全局上下文。
            stream_callback: 可选流式回调。

        返回:
            List[Dict]: 子节提纲数组，每个元素包含 subsection_id, title, target_words 等。
        """
        from ..prompts import SYSTEM_PROMPT_CHAPTER_OUTLINE, build_chapter_outline_prompt
        ...
        payload = {
            ...
            "chapterPlan": chapter_plan,
            "reports": context.get("reports", {}),
            "forumLogs": context.get("forum_logs", "")[:3000],
        }

        user_message = build_chapter_outline_prompt(payload)
        logger.info(f"[分段生成] 为章节 {section.title} 生成提纲...")
        ...
        try:
            ...
            logger.error(f"章节提纲解析失败: {exc}")
            return []

    def _generate_subsection_content(
        self,
        section: TemplateSection,
        ...
        """
        根据提纲生成单个子节的 blocks 内容。

        参数:
            section: 父章节模板切片。
            subsection: 子节提纲（包含 title, key_points, target_words 等）。
            context: 全局上下文。
            previous_summaries: 前面子节的摘要列表，用于保持连贯性。
            stream_callback: 可选流式回调。

        返回:
            Tuple[List[Dict], str]: (blocks 数组, 本节摘要)
        """
        from ..prompts import SYSTEM_PROMPT_SUBSECTION_CONTENT, build_subsection_content_prompt
        ...
        payload = {
            ...
            "previousSummaries": previous_summaries[-3:] if previous_summaries else [],
            "reports": {
                "query_engine": reports.get("query_engine", "")[:5000],
                "media_engine": reports.get("media_engine", "")[:5000],
                "insight_engine": reports.get("insight_engine", "")[:5000],
            },
            "forumLogs": context.get("forum_logs", "")[:2000],
        }

        user_message = build_subsection_content_prompt(payload)
        subsection_title = subsection.get("title", "未命名子节")
        logger.info(f"[分段生成] 生成子节: {subsection_title}")
        ...
        try:
            ...
            # 尝试修复常见 JSON 问题
            cleaned = self._repair_llm_json(cleaned)
            
            try:
                data = json.loads(cleaned)
            except json.JSONDecodeError:
                # 尝试使用 RobustJSONParser
                try:
                    data = self._robust_parser.parse(
                        cleaned,
                        context_name="SubsectionContent",
                        expected_keys=["blocks", "summary"],
                    )
                except Exception:
                    # 最后尝试 json_repair
                    if _json_repair_fn:
                        repaired = _json_repair_fn(cleaned)
            ...
            logger.info(f"[分段生成] 子节 {subsection_title} 生成完成，{len(blocks)} 个 blocks")
            return blocks, summary
        except (json.JSONDecodeError, KeyError, Exception) as exc:
            logger.error(f"子节内容解析失败: {exc}")
            # 返回一个占位 paragraph 而不是空数组
            placeholder_block = {
                "type": "paragraph",
                "inlines": [{"text": f"[子节 {subsection_title} 内容生成失败，请重试]", "marks": []}]
            }
            return [placeholder_block], ""

    def _merge_subsection_blocks(
        self,
        section: TemplateSection,
        all_blocks: List[List[Dict[str, Any]]],
    ) -> Dict[str, Any]:
        """
        将多个子节的 blocks 合并为完整的章节 JSON。

        参数:
            section: 章节模板切片。
            all_blocks: 各子节的 blocks 数组列表。

        返回:
            Dict: 完整的章节 JSON 结构。
        """
        merged_blocks: List[Dict[str, Any]] = []
        for subsection_blocks in all_blocks:
            ...
            for block in subsection_blocks:
                ...
                elif block and isinstance(block, str) and len(block.strip()) > 2:
                    # 非空字符串转为 paragraph（忽略单个字符如换行符）
                    merged_blocks.append(self._as_paragraph_block(block.strip()))
        ...
        logger.info(f"[分段生成] 章节 {section.title} 合并完成，共 {len(merged_blocks)} 个 blocks")
        return chapter_json
    ...
    def run_segmented(
        ...
        **kwargs,
        ...
        # Step 1: 生成提纲
        outline = self._generate_chapter_outline(section, context, stream_callback)
        if not outline:
            logger.warning(f"[分段生成] 章节 {section.title} 提纲生成失败，回退到整体生成")
        ...
        for subsection in outline:
            blocks, summary = self._generate_subsection_content(
                section, subsection, context, summaries, stream_callback
            )
        ...
        # Step 3: 合并
        chapter_json = self._merge_subsection_blocks(section, all_blocks)
        ...
        if not valid and errors:
            repaired = self._attempt_llm_structural_repair(chapter_json, errors)
            if repaired:
                chapter_json = repaired
                self._sanitize_chapter_blocks(chapter_json)
                valid, errors = self.validator.validate_chapter(chapter_json)
